...
const PROMPT_TEXT = `You are an expert data analyst specializing in professional profiles. Your task is to parse the raw text from a LinkedIn profile and extract structured data according to the provided schema.
...
export async function parseLinkedInProfile(
  ...
  if (!output.profession && output.experience?.length) {
