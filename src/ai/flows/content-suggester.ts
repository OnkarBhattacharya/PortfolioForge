 ...
 *
 * - suggestContent - A function that provides suggestions to improve text.
...
export const ContentSuggesterInputSchema = z.object({
  ...
  contentType: z.string().describe('The type of content to improve (e.g., "project description", "summary").'),
});

export const ContentSuggesterOutputSchema = z.object({
  suggestions: z.array(z.string()).describe('A list of AI-generated suggestions to improve the text.'),
});
...
export async function suggestContent({ text, contentType }: ContentSuggesterInput): Promise<ContentSuggesterOutput> {
...
User's Text: "${text}"`,
    output: { schema: ContentSuggesterOutputSchema },
    config: { temperature: 0.7 },
  });

  if (!output) {
    throw new Error('Failed to generate suggestions. The model did not return a valid response.');
  }
  return output;
}

// Preserved export alias for existing API route consumers
export const contentSuggesterFlow = suggestContent;
