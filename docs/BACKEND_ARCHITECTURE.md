...
| `itemIndex` | number | Position index; used to enforce free-plan limit of 3 |

> All portfolio item creation goes through `POST /api/portfolio-items` (Admin SDK). Direct client writes are blocked by Firestore rules (`allow create: if false`).
...
- Admin users (role: `'admin'`) can list all `/users/` documents for the admin dashboard.
- Free-plan users cannot set `themeId` to a premium theme.
- Free-plan users cannot write `customDomain` or `customTheme`.
- `/themes/` is publicly readable; writes are blocked for all clients.
- `/users/{userId}/portfolioItems/` — client creates are blocked (`allow create: if false`); all creates go through the server-side `/api/portfolio-items` route which enforces the free-plan limit of 3.
...
| `POST /api/github-importer` | `github-importer.ts` | Fetches repos, summarises READMEs via `readme-summarizer.ts`, writes to `portfolioItems` |
...
| `POST /api/portfolio-items` | — | Creates a `PortfolioItem` via Admin SDK; enforces free-plan limit of 3 |
...
> In production, Stripe secrets must be provisioned in Google Cloud Secret Manager and referenced in `apphosting.yaml`. Until they are set, the Stripe routes return HTTP 500 with "Stripe is not configured".
...
## 8. App Hosting configuration

`apphosting.yaml` configures the Firebase App Hosting backend:

- Min 1 / max 10 instances, 1 vCPU, 512 MiB memory, 80 concurrent requests.
- VPC connector (`managed-vpc`) for private network access.
- `NODE_ENV=production` and non-sensitive Firebase client config committed as plain values.
- `GOOGLE_GENAI_API_KEY` and `FIREBASE_SERVICE_ACCOUNT_KEY` resolved from Secret Manager at runtime.
- Stripe secrets are defined in the file but commented out; uncomment and populate in Secret Manager to enable Stripe in production.
